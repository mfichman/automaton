...
function compile_template_to_table(result, data, start_lua, end_lua)
...
function compile_template_as_function(data, start_lua, end_lua)
...
function compile_template(data, start_lua, end_lua)
  return
  table.concat(compile_template_to_table({ }, data, start_lua, end_lua))
end

function load_template(data, start_lua, end_lua)
  return
  assert(loadstring(compile_template_as_function(data, start_lua, end_lua),
                    "=(load)"))()
end

function execute_template(template, environment, output)
...
function load_environment(env_files, merge_global, environment)
  ...
  if env_files and #env_files > 0 then
    for i = 1,#env_files,1 do
      ...
      if i > 1 then
        environment = merge_table(setfenv(efun, environment)(), environment)
      else
        environment = basic_environment(merge_global, efun())
      end
    end
  else
    environment = basic_environment(merge_global, environment)
  end
...
function read_template(template)
  return ((template == "-" and io.stdin:read("*a")) or read_all(template))
end

function render_template(template_data, start_lua, end_lua, environment)
...
function execute_env_code(env_code, environment)
  for i = 1,#env_code do
...
function render(outfile, num_passes, template, merge_global,
                      env_files, start_lua, end_lua, env_code, env)
  local data = assert(read_template(template), "error reading " .. template)
  local environment = load_environment(env_files, merge_global, env)
...
function render_to_file(outfile, template, env)
  render(io.output(outfile), 1, template, nil, nil, "<?lua", "?>", {}, env)
end

function compile_as_function(outfile, template, start_lua, end_lua)
  local data = read_template(template)
  outfile:write(compile_template_as_function(data, start_lua, end_lua))
end

function compile(outfile, template, start_lua, end_lua)
  local data = read_template(template)
